// get users
let uri="http://localhost:3030/api/donation/"

let HospitalUrl="http://localhost:3030/api/hospital/"
let resultUrl="http://localhost:3030/api/result/"
    ...
    $("#Dhospial").empty()
     ...
     $.ajax({  
        ...
        success: function (data ) {  
            //console.log(data);
            $("#Dresult").append(`
            <option value="" selected disabled>Test Result</option>
                `)
            $.each(data, function(idx , item){
                // console.log(item);
                $("#Dresult").append(`
                <option value="${item.id}" > ${item.donor.name } | ${item.isHealthy} | ${item.date }  </option>
                `)
      ...
      $.ajax({  
        ...
        success: function (data ) {  
            ...
            $.each(data, function(idx , item){
                ...
                $tr.append("<td>"+item.results.donor.name+"</td>");
                $tr.append("<td>"+item.results.donor.bloodType.bloodName+"</td>");

                $tr.append("<td>"+item.quantity +" ML"+"</td>");
